...
void BossJump::Update()
{
	float deltaTime = m_pOwner->GetDelta();

	switch (m_List)
	{
	case BossJump::enJump::None:
		m_Velocity = {};
		// 当たり判定を有効化.
		m_List = enJump::Charge;

		break;
	case BossJump::enJump::Charge:
		ChargeTime();

		break;
	case BossJump::enJump::Jump:
		JumpTime();
		break;
	case BossJump::enJump::Attack:
		BossAttack();
		break;
	case BossJump::enJump::CoolDown:
		m_Timer += deltaTime;
...
std::pair<Boss::enBossAnim, float> BossJump::GetParryAnimPair()
{
	return std::pair<Boss::enBossAnim, float>();
}


void BossJump::JumpTime()
{
...
void BossJump::BossAttack()
{
    ...
    if (DistanceToPlayer > trackingThreshold)
    {
        ...
        {
            DirectX::XMVECTOR rotationAxis = DirectX::XMVector3Cross(CurrentDir, TargetStepDir);
